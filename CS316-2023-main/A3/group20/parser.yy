...
function_definition: 
     ...
     {
          ast.insert({$3->name, $5});
          gst->entries.insert({$3->name, new SymbTabEntry($3->name, FUN_TYPE, GLOBAL_SCOPE, offset, 0, $3->type->type, lst)});
     }
     ;
...
main_definition:
     INT MAIN '(' ')' { lst = new SymbTab(); offset = LOCAL_OFFSET; func_params[$2]; entity_type = {FUN_TYPE, new decl_struct($2, new type_struct(INT_TYPE, INT_SIZE))}; } compound_statement
     {
          ast.insert({$2, $6});
          gst->entries.insert({$2, new SymbTabEntry($2, FUN_TYPE, GLOBAL_SCOPE, offset, 0, INT_TYPE, lst)});
     }
     ;
...
printf_call:
     PRINTF '(' STRING_LITERAL ')' ';'
     {
          ...
          params.insert(params.begin(), new stringconst_astnode(STRING_TYPE, $3));
	     $$ = new proccall_astnode(new identifier_astnode(PREDEFINED.at($1), $1), params);
     }
     | PRINTF '(' STRING_LITERAL ',' expression_list ')' ';'
     {
          std::vector<exp_astnode*> params = $5;
          params.insert(params.begin(), new stringconst_astnode(STRING_TYPE, $3));
          $$ = new proccall_astnode(new identifier_astnode(PREDEFINED.at($1), $1), params);
     }
     ;
