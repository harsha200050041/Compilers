    ...
    if (this->entries.size())
    {
        ...
        for (auto it = this->entries.begin(); it != this->entries.end(); it++)
        {
            ...
            std::cout << blank_string << BLANK_STRING << "[\n"
                      ...
                      << blank_string << BLANK_STRING << BLANK_STRING << (it->second->varfun == STRUCT_TYPE ? "\"-\"" : (it->second->varfun == FUN_TYPE ? "0" : std::to_string(it->second->offset))) << ",\n"
                      << blank_string << BLANK_STRING << BLANK_STRING << "\"" << (it->second->varfun == STRUCT_TYPE ? "-" : it->second->type) << "\"\n"
